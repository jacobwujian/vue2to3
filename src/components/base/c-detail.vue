  ...
  <div>
    <div v-for="(lItem, lIndex) in form" :key="lIndex">
      <a-descriptions :title="lItem.title" :column="lItem.children ? 1 : column || 3" :layout="type">
        <template v-if="lItem.children">
          <a-descriptions-item v-for="(sItem, index) in lItem.children" :key="index">
            <a-descriptions :title="sItem.title" :layout="type">
              <a-descriptions-item v-for="(ssItem, ssIndex) in sItem.list" :key="ssIndex + lIndex" :label="ssItem.label">
        ...
        <template v-else>
          <a-descriptions-item v-for="(sItem, sIndex) in lItem.list" :key="sIndex + lIndex" :label="sItem.key.includes('approve') || sItem.key.includes('fileInfos') ? '' : sItem.label">
            ...
            <template v-else-if="sItem.type === 'approveRemark'">
              <a-form-model ref="approveRemarkRef" :model="sItem">
                <a-form-model-item prop="value" label="审核备注" :rules="reviewRules1.value">
                  <a-textarea v-model.trim="sItem.value" property="approveRemark" :disabled="sItem.formType !== 'review'" :auto-size="{ minRows: 3, maxRows: 5 }" placeholder="请输入审核备注" />
                </a-form-model-item>
              </a-form-model>
            </template>
              ...
              <a-form-model ref="valueRef" :model="sItem">
                <a-form-model-item prop="value" label="审核结果" :rules="reviewRules.value">
                  <a-radio-group v-model="sItem.value" :disabled="sItem.formType !== 'review'">
                    <a-radio v-for="(op, i) in sItem.dataSource" :key="`${i}-${op.value}`" :value="op.value">
                      {{ op.name }}
                    </a-radio>
                  </a-radio-group>
                </a-form-model-item>
...
export default {
  ...
  data() {
    return {
      codeToText,
      reviewRules: {
        value: [{ required: true, message: '请选择审核结果', trigger: 'change' }]
      },
      reviewRules1: {
        value: [{ required: true, message: '请输入审核备注', trigger: 'blur' }]
      }
    }
  },
  methods: {
